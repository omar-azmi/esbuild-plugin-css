...
import { copyUrlFileFilter, copyUrlNamespace, pluginFilter, pluginName, pluginNamespace, preserveUrlFilters, preserveUrlNamespace } from "./typedefs.js";
...
*/
const css_importer_map = new Map();
/** esbuild plugin that bundles css files. compatible with deno and browsers. */
export const cssPlugin = (options = {}) => {
    const { cache = new Map(), filter = pluginFilter, preserveUrl = preserveUrlFilters, copyUrl = copyUrlFileFilter, } = options;
    ...
    return {
        ...
        setup: (build) => {
            const pathResolver = (args) => {
                ...
                const { path, importer, kind } = args, path_url_str = resolveAsUrl(path, kind === "entry-point" ? undefined : importer).toString();
                if (DEBUG.LOG) {
            ...
            preserveUrl.forEach((filter_regex) => build.onResolve({ filter: filter_regex, namespace: pluginNamespace }, async (args) => {
                ...
                if (DEBUG.ASSERT) {
                    ...
                    console.assert(kind === "import-rule" || kind === "url-token", "the importer of the following path is not a css file (not a css import-rule)", "\n\tpath:", path, "\n\importer:", importer, "\n\kind:", kind);
                }
                if (DEBUG.LOG) {
            ...
            build.onLoad({ filter: /.*/, namespace: pluginNamespace }, async (args) => {
                ...
                if (options.mode === "inject" && importer_kind === "import-statement" || importer_kind === "dynamic-import") {
                    ...
                    let css_content = cached_contents;
